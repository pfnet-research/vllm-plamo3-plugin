...
class Plamo3AttentionMixer(nn.Module):
    ...
    def __init__(self,
        ...
        self.attn = Attention(
            ...
            per_layer_sliding_window=config.
            interleaved_sliding_window[layer_idx],
    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        q = q.reshape(q_shape[:-1] +
                      (q_shape[-1] // self.head_dim, self.head_dim))
        q = self.q_norm.forward_native(q).reshape(q_shape)
        k_shape = k.shape
        k = k.reshape(k_shape[:-1] +
                      (k_shape[-1] // self.head_dim, self.head_dim))
...
class Plamo3Model(nn.Module):
    ...
    def forward(
        ...
        hidden_states, residual = self.layers(positions=positions,
                                              hidden_states=hidden_states,
                                              residual=residual)
...
class Plamo3ForCausalLM(nn.Module, SupportsPP):
    ...
    def compute_logits(
        self,
        hidden_states: torch.Tensor,
        *args: Any,  # For backward compatibility with vLLM<0.11.0.
    ) -> torch.Tensor | None:
        logits = self.logits_processor(self.lm_head, hidden_states, *args)
        return logits

    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]):
        params_dict = dict(self.named_parameters())
        for name, loaded_weight in weights:
            # Skip inv_freq since we always recompute inside RotaryEmbedding.
            if ".mixer.rotary_emb.inv_freq" in name:
                continue
            ...
            if ".q_norm" in name or ".k_norm" in name or ".pre_mixer_norm" in name:
                loaded_weight += 1.0
